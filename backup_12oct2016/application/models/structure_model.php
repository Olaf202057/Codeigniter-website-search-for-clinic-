...
class Structure_model extends MY_Model
{
	...
	public function get_structure_exam($limit=0, $start=0,$return_type='') {
		...
		$get =$this->input->get();
		$sql = "SELECT s.id, e.id as eid, e.id as exam_id, s.image_url as imageurl, s.hospital, s.hospital as naming_struct, s.address_formatted as address, s.latitude, s.longitude, s.city, s.province, s.telephone, s.website as website_int_struct, e.official_waiting_days, e.exam_type as exam_type, e.price as price, s.website as website FROM tblstructure s, tblexamination e where 1 AND s.id=e.struct_id ";		
