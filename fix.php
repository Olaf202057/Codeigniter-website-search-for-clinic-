...
$sqla = "SELECT * FROM `tblstructure` WHERE `address_formatted` LIKE 'Milan, Italy' AND fixed='0' ORDER BY id ASC LIMIT 0,1";
...
if ($howmanya > 0) {
	while ($rowa = mysql_fetch_array($resulta)) {
		...
		//exit;
		if ($gmap_array[1] != '') {
			$sqlc = "UPDATE tblstructure SET latitude='".$gmap_array[0]."', longitude='".$gmap_array[1]."', address_formatted='".addslashes($gmap_array[2])."', fixed='1' WHERE id = '".$rowa['id']."'";
			$resultc = mysql_query($sqlc) or die ("Error in query: $sqlc. ".mysql_error());
			echo '<meta http-equiv="refresh" content="2">';
			exit;
		} else {
			$sqlc = "UPDATE tblstructure SET fixed='1' WHERE id = '".$rowa['id']."'";
			$resultc = mysql_query($sqlc) or die ("Error in query: $sqlc. ".mysql_error());
			echo '<meta http-equiv="refresh" content="2">';
			exit;
		}
