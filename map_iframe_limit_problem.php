...
if ($conn)
{
	...
	$query = "SELECT s.id, e.id as eid, s.hospital, s.address, s.city, s.province, s.website, e.official_waiting_days FROM tblstructure s, tblexamination e where e.id IN (".$exam_ids.") AND s.id=e.struct_id LIMIT 10";
	...
	while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
		...
		if ($address != '') {
			$address_line = '[\''.addslashes(trim($row['hospital'])).'\', \''.addslashes(trim($address)).'\', \'Waiting Time: '.addslashes(trim($row['official_waiting_days'])).' days\']';
