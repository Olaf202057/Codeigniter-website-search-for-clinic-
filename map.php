...
if ($conn)
{
	$query = "SELECT s.id, e.id as eid, s.hospital, s.address, s.city, s.province, s.website, e.official_waiting_days FROM tblstructure s, tblexamination e where exam_type LIKE 'ESAME DEL FUNDUS OCULI' AND s.id=e.struct_id LIMIT 0,10";
	$result = $conn->prepare($query);
	$result->execute();
	     
	/* Retrieve and store in array the results of the query.*/
	while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
		$address_line = $row['address'].' '.$row['city'].' '.$row['province'];
		$address_line = '[\''.addslashes($row['hospital']).'\', \''.addslashes($address_line).'\', \'Waiting Time: '.addslashes($row['official_waiting_days']).' days\']';
