...
class Home extends MY_Controller {
	...
	{
		$get =$this->input->get();
		//$post_data = array("actual_time" => 1, "comment" => "test", "exam_id" => 78, "rating" => 2, "structure_id" => 9);
		//$post_data = array("actual_time" => 19999, "comment" => "test", "exam_id" => 13417, "rating" => 2, "structure_id" => 9);
		
		//$this->emod->save_review($post_data);
		...
		{
			if($hosp->owner_id == 0 || $hosp->owner_id =='')
			{
				continue;
			}
			else
		...
		if($this->input->is_ajax_request()){
			$data_temp = array('structures' => $structure);
			$html = $this->load->view('home/ajax-search-results-view', $data_temp, true);
			$data['html'] = $html;
	...
	public function hospitals($id)
	{
		...
		//echo "<pre>"; print_r($data['structure']); echo "</pre>";
		//echo "<pre>"; print_r($data['exam']); echo "</pre>";
		if($data['exam']->owner_id != 0)
		{
	...
	public function city() {
		...
		$this->db->from('tblexamination');
		$this->db->order_by('exam_type', 'ASC');
        $query1 = $this->db->get();
		$data_exam = $query1->result();

		$this->db->select('name');
		$this->db->from('cities');
		$this->db->order_by('name', 'ASC');
        $query2 = $this->db->get();
		$data_city = $query2->result();

		$this->output->set_content_type('application/json');
        $result = array('data_exam'=>$data_exam, 'data_city'=>$data_city);
