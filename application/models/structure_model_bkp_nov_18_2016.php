...
class Structure_model extends MY_Model
{
	...
	public function get_structure_exam($limit=0, $start=0,$return_type='') {
		...
		$sql1 = ' SELECT   e.source,(SELECT IFNULL(AVG(IFNULL(rating,0)),0) FROM  `tblexamination_review` WHERE `status`=1 AND exam_id=e.id) AS ratings,  e.common_name,s.id,e.owner_id,e.owner_id as clicks_amount,e.owner_id as budget_amount,e.owner_id as remianing_amount,e.id as eid, e.id as exam_id, s.image_url as imageurl, s.hospital, s.hospital as naming_struct,s.address_formatted as address, s.latitude, s.longitude, s.city, s.province, s.telephone, s.website as website_int_struct, e.official_waiting_days, e.exam_type as exam_type, e.price as price, s.website as website FROM tblstructure s,tblexamination e WHERE  s.id=e.struct_id AND e.owner_id = 0';
				if($get){
			if(isset($get['q']) && $get['q']!=""){
