<style type="text/css">
.fixed-block-container{position: fixed; width:100%;z-index: 999;}
</style>

<div class="profile-section fixed-block-container">
     <div class="container">
        <div class="row">
           <div class="profile-tab collapse navbar-collapse" id="bs-example-navbar-collapse-1">
              <ul>
                 <!-- <li <?php if($this->uri->segment(1)==''){?> class="act-tab" <?php }?>>
                    <a href="<?php echo base_url();?>">
                    <span>Home</span>
                    </a>
                 </li> -->
                  <li>
                    <a href="<?php echo base_url();?>ads/dashboard" <?php if($this->uri->segment(1)=='ads' && $this->uri->segment(2)=='dashboard'){?> class="act-tab" <?php }?>>
                    <span>Risultati</span>
                    </a>
                 </li>
                 <li>
                    <a href="<?php echo base_url();?>ads/examination" <?php if($this->uri->segment(1)=='ads' && $this->uri->segment(2)=='examination'){?> class="act-tab" <?php }?>>
                    <span>Dettaglio statistiche</span>
                    </a>
                 </li>
                 <li>
                    <a href="<?php echo base_url();?>ads/opportunities" <?php if($this->uri->segment(1)=='ads' && $this->uri->segment(2)=='opportunities'){?> class="act-tab" <?php }?>>
                    <span>Opportunità</span>
                    </a>
                 </li>
                <!--  <li>
                    <a href="<?php echo base_url();?>ads/analytics" <?php if($this->uri->segment(1)=='ads' && $this->uri->segment(2)=='analytics'){?> class="act-tab" <?php }?>>
                    <span>tools and analytics</span>
                    </a>
                 </li> -->
                 <li>
                    <a href="<?php echo base_url();?>exam" <?php if($this->uri->segment(1)=='exam'){?> class="act-tab" <?php }?>>
                    <span>Strumenti</span>
                    </a>
                 </li>
                 <li>
                    <a href="<?php echo base_url();?>ads/myorders" <?php if($this->uri->segment(1)=='ads' && $this->uri->segment(2)=='myorders'){?> class="act-tab" <?php }?>>
                    <span>Transazioni</span>
                    </a>
                 </li>
                  <li>
                    <a href="<?php echo base_url();?>ads/balance" <?php if($this->uri->segment(1)=='ads' && $this->uri->segment(2)=='balance'){?> class="act-tab" <?php }?>>
                    <span>Credito</span>
                    </a>
                 </li>
                 <li>
                 <a href="<?php echo base_url();?>home/myaccount" <?php if($this->uri->segment(1)=='home' && $this->uri->segment(2)=='myaccount'){?> class="act-tab" <?php }?>>
                    <span>Profilo</span>
                    </a>
                 </li>
                 <?php
                  if(is_admin())
                  {
                  ?>
                  <li>
                    <a href="<?php echo base_url() ?>admin" <?php if($this->uri->segment(1)=='ads' && $this->uri->segment(2)=='admin'){?> class="act-tab" <?php }?>>
                    <span>Admin Panel</span>
                    </a>
                 </li>   
                  <?php
                  }
                  ?>
                 <?php if(!is_login(false)){ ?>
                 <li>
                    <a href="<?php echo base_url() ?>ads">
                    <span>Get started</span>
                    </a>
                 </li>
                 <?php } ?>
                 <li class="visible-xs">
                    <div class="navbar-collapse" id="navbar">
                    	<?php if(is_login(false)){ 

                          if(isset($this->session->userdata['logout_url']) && $this->session->userdata('logout_url') !='')
                          {
                            $url = $this->session->userdata('logout_url');
                          }
                          else 
                          {
                            $url = base_url().'disconnessione';
                          }
                       ?>
                       <ul class="nav navbar-nav">
                          <li class="dropdown">
                             <a href="#" class="dropdown-toggle" data-toggle="dropdown"><?php echo get_user_fullname(); ?><span class="caret"></span></a>
                             <ul class="dropdown-menu">
                                <li><a href="<?php echo base_url() ?>home/myaccount">Account Settings <span class="glyphicon glyphicon-cog pull-right"></span></a></li>
                                <?php if($this->session->userdata('user')->role != 3){ ?>
                                <li><a href="<?php echo base_url();?>ads/dashboard">Risultati <span class="glyphicon glyphicon-stats pull-right"></span></a></li>
                                <?php } ?>
                                <!--  <li><a href="#">Messages <span class="badge pull-right"> 42 </span></a></li>
                                <li><a href="#">Favourites Snippets <span class="glyphicon glyphicon-heart pull-right"></span></a></li> -->
                                <li><a href="<?php echo $url; ?>">disconnessione <span class="glyphicon glyphicon-log-out pull-right"></span></a></li>
                             </ul>
                          </li>
                       </ul>

                        <?php if(is_admin())
                           { ?>
                           	 <ul class="nav navbar-nav navbar-right">
                              <li class="dropdown">
                                 <a href="<?php echo base_url() ?>admin" class="dropdown-toggle" data-toggle="dropdown1"> Pannello di controllo<span class="caret1"></span></a>
                              </li>
                             </ul>
                           <?php }
                           }?>
                       <ul class="nav navbar-nav navbar-right">
                       	<?php if(!is_login(false)){ ?>
                          <li><a href="<?php echo base_url() ?>accesso">Accesso</a></li>
                          <?php } ?>
                          <li class="dropdown">
                             <a aria-expanded="true" aria-haspopup="true" href="#" data-toggle="dropdown" class="dropdown-toggle"> Lingua <span class="caret"></span></a> 
                             <ul class="dropdown-menu">
                                <li><a href="#"><img alt="" src="<?php echo base_url() ?>assets/img/Italy-Flag.png"> Italian</a></li>
                                <li><a href="#"><img alt="" src="<?php echo base_url() ?>assets/img/USA-Flag.png"> English</a></li>
                             </ul>
                          </li>
                       </ul>
                    </div>
                 </li>
              </ul>
           </div>
        </div>
     </div>
</div>